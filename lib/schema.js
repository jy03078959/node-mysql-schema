...
Schema.prototype.encodeSchema = function (object) {
  ...
  for (let key in object) {
    ...
    let tmp = key.split('$')
    field = tmp[0]
    op = (tmp[1] || '').toUpperCase()
    if (!field && op) {
      result[key] = this.encodeSchema(object[key])
    } else if (!this._schemas[field]) {
      continue
    } else if (op === 'IN' && Array.isArray(object[key])) {
      result[key] = object[key].map(function (val) {
        return self._schemas[field].encode(val)
      })
    } else if (object[key] !== undefined) {
...
Schema.prototype.aggregate = function *(fields, cond, group, options) {
  cond = this.encodeSchema(cond)
  options = options || {}
